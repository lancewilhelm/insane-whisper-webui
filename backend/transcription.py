...
def transcribe_audio(file_path, model_name, diarization_model, diarization_options, hf_token, device_id):
    ...
    # Perform diarization if options are provided
    if diarization_options:
        ...
        diarization_pipeline.to(torch.device(
            "mps" if device_id == "mps" else f"cuda:{device_id}"))
        ...
        with Progress(
            ...
            inputs, diarizer_inputs = preprocess_inputs(inputs=file_path)
            segments = diarize_audio(
                diarizer_inputs,
                diarization_pipeline,
                diarization_options.get('num_speakers'),
                diarization_options.get('min_speakers'),
                diarization_options.get('max_speakers')
            )
            speakers_transcript = post_process_segments_and_transcripts(
